...
# "fatorizar" -> irá ordenar e listar os fatores gerados pela função anterior para apresentar;
# "main" -> função main;
...
def eh_primo(n, k=10):
    ...
    # o objetivo é puramente para fins de otimização:
    ...
    # pensemos no quadrado da soma pela diferença: x^2-y^2 = (x+y)(x-y)
    ...
    # note que podemos tomar (a^{(n-1)/2} - 1) e fatorar novamente como quadrado da soma pela diferença, seguindo o padrão de aumentar a potência do denominador 2 conforme fatoramos (já que sempre que ocorre uma fatoração, é necessário dividir por 2)
    ...
    for i in range(k): # queremos realizar o teste k vezes para fugir dos problemas probabilisticos

        if n <= 3:
            return True # 2 e 3 já são primos, ora pois
...
def pollards_rho(n, iteracao=0):
    ...
    # antes de tudo, é melhor explicar que para evitar recursões infinitas, definiremos um número máximo de tentativas
    ...
    if iteracao > 50: # basicamente, se atingirmos 50 tentativas, ao invés de fatorar com pollard's rho, tentaremos com força bruta
        limite = min(int(math.sqrt(n)) + 1, 1000)
